...
- A tool-enabled session may rebuild you with `rebuild-ollama-models codex-nemotron --profile <name>`.
- Outcome memory is recorded with `model-outcome record --model codex-nemotron --task <task> --outcome <success|failure|mixed|note> --quality <1-5> --note <text>`.
